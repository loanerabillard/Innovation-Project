...
from Normalisation_player import normalisation_player
...
def calculate_win_probability(player1, player2, surface, merged_df, stat):
    ...
    # Récupération des statistiques globales des joueurs
    if (len(stat[stat['Joueur'] == player1]) == 0) or (len(stat[stat['Joueur'] == player2]) == 0):
        return None, None 
    player1_stats = stat[stat['Joueur'] == player1].iloc[0]
    player2_stats = stat[stat['Joueur'] == player2].iloc[0]

    # Calcul des paramètres pour les joueurs
    rang_diff = player2_stats['Rang'] - player1_stats['Rang']
    vic_surface1 = player1_stats[f'% victoire {surface}']
    vic_surface2 = player2_stats[f'% victoire {surface}']
    vic_general1 = player1_stats['% victoire général']
    vic_general2 = player2_stats['% victoire général']
    ...
    gamma = 0.3
    delta = 0.2
    epsilon = 0.07  # Valeur choisie pour l'effet de l'âge

    # Fonction g(Age)
    def g(age):
        return 1 / (abs(age - 27) + 1)

    # Calcul des scores de base sans confrontation directe
    score1 = alpha * rang_diff + beta * player1_stats['% victoire général'] + gamma * vic_surface1 + delta * vic_general1 + epsilon * g(player1_stats['Age'])
    score2 = -alpha * rang_diff + beta * player2_stats['% victoire général'] + gamma * vic_surface2 + delta * vic_general2 + epsilon * g(player2_stats['Age'])

    # Ajustement si les joueurs se sont déjà rencontrés
    previous_matches = merged_df[(merged_df['Joueur'] == player1) & (merged_df['Adversaire'] == player2)]
    if not previous_matches.empty:
        previous_match = previous_matches.iloc[0]
        vic_adversaire1 = previous_match['% Victoire']
        vic_adversaire2 = 100 - vic_adversaire1  # Complémentaire de la victoire de l'adversaire
        vic_surface_adversaire1 = previous_match[f'% Victoire ({surface})']
        vic_surface_adversaire2 = 100 - vic_surface_adversaire1  # Complémentaire sur la surface

        score1 += beta * vic_adversaire1 + gamma * vic_surface_adversaire1
        score2 += beta * vic_adversaire2 + gamma * vic_surface_adversaire2

    # Calcul des probabilités de victoire en utilisant la formule logistique
    Z = score1 - score2
    win_prob_player1 = 1 / (1 + np.exp(-Z))
    win_prob_player2 = 1 - win_prob_player1

    # Conversion des probabilités en pourcentages
    win_prob_player1 *= 100
    win_prob_player2 *= 100
...
def get_win_probabilities():
    ...
    merged_df = pd.merge(stats_globales_df, stat, on='Joueur', how='outer')
    ...
    for index, row in merged_df.iterrows():
        ...
        for surface in surfaces:
            ...
            win_prob_player1, win_prob_player2 = calculate_win_probability(player1, player2, surface, merged_df, stat)

            if (win_prob_player1, win_prob_player2) != (None, None):
    ...
    pd.DataFrame(win_probabilities).to_csv('back-end/data/win_probabilites_surfaces.csv')
    print(len(win_probabilities))
    print(win_probabilities[4])
    print(f'Catch : {i}')
    print(f'Miss : {missing_count}')
